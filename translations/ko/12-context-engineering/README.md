...
AI 에이전트를 구축하려는 애플리케이션의 복잡성을 이해하는 것은 신뢰할 수 있는 에이전트를 만드는 데 중요해요. 우리는 프롬프트 엔지니어링을 넘어 복잡한 요구를 해결하기 위해 정보를 효과적으로 관리하는 AI 에이전트를 구축해야 해요.

이번 강의에서는 컨텍스트 엔지니어링이 무엇인지, 그리고 AI 에이전트 구축에서 그 역할에 대해 알아볼게요.

## 이번 강의 내용

- **컨텍스트 엔지니어링이란?** 그리고 프롬프트 엔지니어링과 어떻게 다른지
- **효과적인 컨텍스트 엔지니어링 전략**: 정보를 작성, 선택, 압축, 분리하는 방법
...
- **컨텍스트 엔지니어링 정의**와 프롬프트 엔지니어링과의 차이점 이해
- **LLM 애플리케이션에서 컨텍스트의 주요 구성 요소 식별**
- **컨텍스트 작성, 선택, 압축, 분리 전략 적용**으로 에이전트 성능 개선
- **컨텍스트 실패 사례 인식** (오염, 산만함, 혼란, 충돌) 및 완화 기술 구현

## 컨텍스트 엔지니어링이란?

AI 에이전트에서 컨텍스트는 에이전트가 특정 행동을 계획하도록 유도하는 핵심 요소예요. 컨텍스트 엔지니어링은 AI 에이전트가 작업의 다음 단계를 완료하는 데 필요한 올바른 정보를 갖추도록 하는 실천이에요. 컨텍스트 윈도우는 크기가 제한되어 있어서, 에이전트 개발자로서 우리는 컨텍스트 윈도우에 정보를 추가, 제거, 압축하는 시스템과 프로세스를 구축해야 해요.

### 프롬프트 엔지니어링 vs 컨텍스트 엔지니어링

| 구분 | 프롬프트 엔지니어링 | 컨텍스트 엔지니어링 |
|------|-------------------|-------------------|
| 초점 | 정적 지침 세트 | 동적 정보 세트 관리 |
| 범위 | 에이전트를 효과적으로 안내 | 초기 프롬프트 포함 + 시간에 따른 정보 관리 |
| 목표 | 효과적인 안내 | 반복 가능하고 신뢰할 수 있는 프로세스 |
...
컨텍스트는 단일 요소가 아니에요. AI 에이전트가 관리해야 할 컨텍스트 유형:

| 유형 | 설명 |
|------|------|
| **지침** | 에이전트의 "규칙": 프롬프트, 시스템 메시지, few-shot 예제, 도구 설명 |
| **지식** | DB에서 검색된 정보, 장기 기억. RAG 시스템 통합 가능 |
| **도구** | 외부 함수, API, MCP 서버의 정의와 사용 결과 |
| **대화 기록** | 사용자와의 진행 중인 대화. 시간에 따라 컨텍스트 윈도우 공간 차지 |
| **사용자 선호도** | 사용자의 선호도(좋아하는 것/싫어하는 것) 학습 및 저장 |
...
좋은 컨텍스트 엔지니어링은 좋은 계획에서 시작해요:

1. **명확한 결과 정의** - "AI 에이전트가 작업을 완료했을 때 세상이 어떻게 변할 것인가?" 즉, 사용자와의 상호작용 후 어떤 변화, 정보, 응답이 있어야 하는지 정의

2. **컨텍스트 매핑** - "AI 에이전트가 이 작업을 완료하기 위해 어떤 정보가 필요한가?" 해당 정보가 어디에 위치할 수 있는지 매핑

3. **컨텍스트 파이프라인 생성** - "에이전트가 이 정보를 어떻게 얻을 것인가?" RAG, MCP 서버 및 기타 도구 사용
