...
- 구조화된 출력을 활용해 더 신뢰할 수 있고 기계가 읽을 수 있는 응답 생성
- 이벤트 기반 접근 방식으로 동적 작업과 예상치 못한 입력 처리
...
| 목표 | 설명 |
|------|------|
| 전체 목표 설정 | AI 에이전트가 무엇을 달성해야 하는지 명확히 |
| 작업 분해 | 복잡한 작업을 관리 가능한 하위 작업으로 분해하고 논리적으로 조직화 |
| 도구 제공 | 적절한 도구(검색 도구, 데이터 분석 도구 등) 제공, 언제/어떻게 사용할지 결정, 예상치 못한 상황 처리 |
| 결과 평가 | 하위 작업 결과 평가, 성과 측정, 최종 출력 개선을 위해 반복 |
...
대부분의 실제 작업은 한 단계로 해결하기엔 너무 복잡해요. AI 에이전트는 계획과 행동을 안내할 간결한 목표가 필요해요. 예를 들어:

> "3일 여행 일정 생성하기."

간단히 표현할 수 있지만, 여전히 세부적으로 다듬어야 해요. 목표가 명확할수록 에이전트(및 인간 협력자)가 올바른 결과를 달성하는 데 집중할 수 있어요. 예: 항공편 옵션, 호텔 추천, 활동 제안이 포함된 포괄적인 일정 만들기.

### 작업 분해

크거나 복잡한 작업은 더 작은 목표 지향적인 하위 작업으로 나누면 관리하기 쉬워져요. 여행 일정 예제를 기준으로:
...
각 하위 작업은 전담 에이전트나 프로세스에 의해 처리될 수 있어요. 한 에이전트는 최적의 항공편 거래를 검색하고, 다른 에이전트는 호텔 예약에 집중하고, 조정 에이전트가 이 결과들을 하나의 통합된 일정으로 사용자에게 제공할 수 있죠.

이 모듈식 접근 방식은 점진적인 개선도 가능하게 해요. 음식 추천이나 지역 활동 제안을 위한 전문 에이전트를 추가해서 일정의 품질을 시간이 지남에 따라 개선할 수 있어요.

### 구조화된 출력

LLM은 구조화된 출력(예: JSON)을 생성할 수 있어서 다운스트림 에이전트나 서비스가 구문 분석하고 처리하기 더 용이해요. 이건 계획 출력이 수신된 후 작업을 실행할 수 있는 다중 에이전트 환경에서 특히 유용해요.

다음 Python 코드 스니펫은 목표를 하위 작업으로 분해하고 구조화된 계획을 생성하는 간단한 계획 에이전트를 보여줘요:
