...
- 데이터와 사용자 프라이버시를 유지하는 방법

## 학습 목표

이 강의를 마치면:

- AI 에이전트의 위험을 식별하고 완화하는 방법 이해
- 데이터와 접근을 관리하기 위한 보안 조치 구현
- 데이터 프라이버시를 유지하고 품질 높은 UX를 제공하는 AI 에이전트 제작

## 안전성

안전한 에이전트 애플리케이션을 구축하는 방법을 살펴볼게요. 안전성이란 AI 에이전트가 설계된 대로 작동하는 것을 의미해요. 에이전트 애플리케이션 개발자로서 안전성을 극대화하기 위한 방법과 도구가 있어요:

### 시스템 메시지 프레임워크 구축

LLM을 사용해 AI 애플리케이션을 구축해봤다면, 견고한 시스템 프롬프트 설계가 얼마나 중요한지 알 거예요. 이런 프롬프트는 LLM이 사용자 및 데이터와 상호작용하는 방식에 대한 메타 규칙, 지침, 가이드라인을 설정해요.

AI 에이전트의 경우, 시스템 프롬프트는 더욱 중요해요. 에이전트가 설계된 작업을 완료하기 위해 매우 구체적인 지침이 필요하기 때문이죠.

확장 가능한 시스템 프롬프트를 만들기 위해 시스템 메시지 프레임워크를 사용할 수 있어요:
...
메타 프롬프트는 LLM이 우리가 생성하는 에이전트의 시스템 프롬프트를 생성하는 데 사용돼요. 템플릿으로 설계해서 필요할 경우 여러 에이전트를 효율적으로 생성할 수 있어요.
