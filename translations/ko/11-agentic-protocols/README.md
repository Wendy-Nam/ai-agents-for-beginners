...
AI 에이전트의 사용이 증가함에 따라 표준화, 보안, 개방형 혁신을 지원하는 프로토콜의 필요성도 커지고 있어요. 이번 강의에서는 이런 요구를 충족시키는 세 가지 프로토콜 - 모델 컨텍스트 프로토콜(MCP), 에이전트 간 통신(A2A), 자연어 웹(NLWeb)을 다룰게요.

## 이번 강의 내용

- **MCP**가 AI 에이전트가 외부 도구와 데이터를 활용하여 사용자 작업을 완료하게 하는 방법
- **A2A**가 서로 다른 AI 에이전트 간의 통신과 협업을 가능하게 하는 방법
- **NLWeb**이 자연어 인터페이스를 웹사이트에 도입해서 AI 에이전트가 콘텐츠를 탐색하고 상호작용하게 하는 방법

## 학습 목표

이 강의를 마치면:

- AI 에이전트의 맥락에서 MCP, A2A, NLWeb의 핵심 목적과 이점을 **식별**할 수 있어요
- 각 프로토콜이 LLM, 도구, 기타 에이전트 간의 통신과 상호작용을 어떻게 촉진하는지 **설명**할 수 있어요
- 복잡한 에이전틱 시스템 구축에서 각 프로토콜의 고유한 역할을 **인식**할 수 있어요

## 모델 컨텍스트 프로토콜

**모델 컨텍스트 프로토콜(MCP)**은 애플리케이션이 LLM에 컨텍스트와 도구를 제공하는 표준화된 방법을 제공하는 개방형 표준이에요. AI 에이전트가 다양한 데이터 소스와 도구에 일관된 방식으로 연결할 수 있는 "범용 어댑터" 역할을 해요.

### MCP 핵심 구성 요소

MCP는 **클라이언트-서버 아키텍처**로 작동해요:

| 구성 요소 | 설명 |
|----------|------|
| **호스트** | LLM 애플리케이션(예: VSCode)으로 MCP 서버와 연결 시작 |
| **클라이언트** | 호스트 내에서 서버와 일대일 연결 유지 |
...
| 요소 | 설명 | 예시 |
|------|------|------|
| **도구** | 에이전트가 호출할 수 있는 개별 작업/기능 | "날씨 조회", "상품 구매" |
| **리소스** | 읽기 전용 데이터 항목/문서 | 파일 내용, DB 레코드, 로그 |
| **프롬프트** | 복잡한 워크플로를 위한 미리 정의된 템플릿 | - |

### MCP의 이점

| 이점 | 설명 |
|------|------|
| **동적 도구 검색** | 에이전트가 사용 가능한 도구 목록을 동적으로 받음. API 변경 시 코드 업데이트 불필요 |
| **LLM 간 상호운용성** | 다양한 LLM에서 작동, 핵심 모델 전환 유연성 |
| **표준화된 보안** | 표준 인증 방법으로 MCP 서버 액세스 추가 시 확장성 개선 |

### MCP 예시

![MCP 다이어그램](../../../translated_images/mcp-diagram.e4ca1cbd551444a12e1f0eb300191a036ab01124fce71c864fe9cb7f4ac2a15d.ko.png)

사용자가 MCP를 활용하는 AI 어시스턴트로 항공편을 예약하고 싶어한다면:

1. **연결**: AI 어시스턴트(MCP 클라이언트)가 항공사 MCP 서버에 연결
2. **도구 검색**: 클라이언트가 "사용 가능한 도구는?" 물음. 서버가 "항공편 검색", "항공편 예약" 응답
3. **도구 호출**: 사용자가 "포틀랜드에서 호놀룰루로 가는 항공편 검색해줘" 요청. AI가 LLM으로 "항공편 검색" 도구 호출 결정, 매개변수를 MCP 서버에 전달
