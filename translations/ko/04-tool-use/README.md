...
도구는 AI 에이전트가 더 넓은 범위의 기능을 가질 수 있게 해주는 흥미로운 요소예요. 에이전트가 수행할 수 있는 행동이 제한적일 때, 도구를 추가하면 다양한 행동이 가능해지죠. 이번 강의에서는 AI 에이전트가 특정 도구를 사용해 목표를 달성하는 도구 사용 디자인 패턴을 살펴볼게요.
...
- 적용 가능한 사용 사례
- 구현에 필요한 요소/구성 요소
- 신뢰할 수 있는 AI 에이전트를 위한 고려 사항

## 학습 목표

이 강의를 마치면:

- 도구 사용 디자인 패턴과 목적을 정의할 수 있어요
- 적용 가능한 사용 사례를 식별할 수 있어요
- 구현에 필요한 주요 요소를 이해할 수 있어요
- 신뢰성 보장을 위한 고려 사항을 인식할 수 있어요

## 도구 사용 디자인 패턴이란?

**도구 사용 디자인 패턴**은 LLM이 외부 도구와 상호작용해서 특정 목표를 달성하게 하는 데 초점을 맞춰요. 도구는 에이전트가 실행할 수 있는 코드로, 계산기 같은 간단한 함수일 수도 있고 주식 가격 조회나 날씨 예보 같은 API 호출일 수도 있어요. AI 에이전트에서 도구는 **모델 생성 함수 호출**에 따라 에이전트가 실행하도록 설계돼요.

## 적용 가능한 사용 사례

AI 에이전트는 도구로 복잡한 작업을 완료하거나 정보를 검색하거나 결정을 내릴 수 있어요. 도구 사용 디자인 패턴은 외부 시스템(데이터베이스, 웹 서비스, 코드 인터프리터 등)과 동적으로 상호작용해야 할 때 자주 사용돼요.

| 사용 사례 | 설명 |
|----------|------|
| **동적 정보 검색** | 외부 API나 DB를 쿼리해 최신 데이터 가져오기 (주식 가격, 날씨 정보 등) |
| **코드 실행 및 해석** | 코드를 실행해 수학 문제 해결, 보고서 생성, 시뮬레이션 수행 |
| **워크플로 자동화** | 작업 스케줄러, 이메일 서비스, 데이터 파이프라인 통합으로 자동화 |
| **고객 지원** | CRM 시스템, 티켓 플랫폼, 지식 기반과 상호작용해 문의 해결 |
| **콘텐츠 생성** | 문법 검사기, 텍스트 요약기, 콘텐츠 안전 평가 도구 활용 |

## 필요한 구성 요소

도구 사용 디자인 패턴을 구현하기 위한 주요 요소:

| 구성 요소 | 설명 |
|----------|------|
| **함수/도구 스키마** | 함수 이름, 목적, 매개변수, 예상 출력이 포함된 상세 정의 |
| **함수 실행 로직** | 사용자 의도와 대화 맥락에 따라 도구 호출을 관리 |
| **메시지 처리 시스템** | 사용자 입력, LLM 응답, 도구 호출/출력 간 대화 흐름 관리 |
| **도구 통합 프레임워크** | 에이전트를 함수나 외부 서비스와 연결하는 인프라 |
| **오류 처리 및 검증** | 도구 실행 실패 처리, 매개변수 검증, 예상치 못한 응답 관리 |
| **상태 관리** | 대화 맥락, 이전 상호작용, 지속적 데이터 추적 |

### 함수/도구 호출

함수 호출은 LLM이 도구와 상호작용하는 주요 방법이에요. '함수'와 '도구'는 종종 같은 의미로 쓰이는데, '함수'(재사용 가능한 코드 블록)가 에이전트가 작업을 수행하는 '도구'이기 때문이에요.

함수 코드가 호출되려면 LLM이 사용자 요청을 함수 설명과 비교해야 해요. 사용 가능한 모든 함수의 스키마가 LLM에 전달되면, LLM은 가장 적합한 함수를 선택하고 함수 이름과 인수를 반환해요. 선택된 함수가 호출되고, 응답이 LLM에 전달되어 사용자 요청에 응답하죠.

에이전트의 함수 호출 구현에 필요한 것:

1. 함수 호출을 지원하는 LLM 모델
2. 함수 설명이 포함된 스키마
3. 설명된 각 함수의 코드

샌프란시스코의 현재 시간을 가져오는 예제로 설명할게요:
