...
- Microsoft Agent Framework로 프로덕션 준비된 AI 에이전트 구축할 수 있어요
- Microsoft Agent Framework의 핵심 기능을 에이전트 기반 사용 사례에 적용할 수 있어요
- 기존 에이전트 기반 프레임워크와 도구를 마이그레이션 및 통합할 수 있어요

## 코드 샘플

[Microsoft Agent Framework (MAF)](https://aka.ms/ai-agents-beginners/agent-framewrok)의 코드 샘플은 이 저장소의 `xx-python-agent-framework` 및 `xx-dotnet-agent-framework` 파일에서 찾을 수 있어요.

## Microsoft Agent Framework 이해하기

![Framework Intro](../../../translated_images/framework-intro.077af16617cf130c0f80f555dbb43cb1066503eaf5a9cc0aa9be67b47722dd52.ko.png)

[Microsoft Agent Framework (MAF)](https://aka.ms/ai-agents-beginners/agent-framewrok)는 Semantic Kernel과 AutoGen에서 얻은 경험과 학습을 기반으로 구축됐어요. 프로덕션 및 연구 환경에서 다양한 에이전트 기반 사용 사례를 처리할 수 있는 유연성을 제공해요.
...
| **마그네틱 오케스트레이션** | 관리자 에이전트가 작업 목록을 생성/수정하고 하위 에이전트를 조정하는 경우 |

### 프로덕션 기능

| 기능 | 설명 |
|------|------|
| **관측 가능성** | OpenTelemetry로 도구 호출, 오케스트레이션 단계, 추론 흐름, Azure AI Foundry 대시보드 성능 모니터링 |
| **보안** | Azure AI Foundry에서 네이티브 호스팅, 역할 기반 액세스, 개인 데이터 처리, 콘텐츠 안전 내장 |
| **내구성** | 에이전트 스레드/워크플로가 일시 중지, 재개, 오류 복구 가능해서 장기 실행 프로세스 지원 |
...
| 특성 | 설명 |
|------|------|
| **클라우드 독립성** | 컨테이너, 온프레미스, 여러 클라우드에서 실행 가능 |
| **프로바이더 독립성** | Azure OpenAI, OpenAI 등 선호 SDK로 에이전트 생성 가능 |
| **오픈 표준 통합** | A2A, MCP 같은 프로토콜로 다른 에이전트 및 도구 발견/활용 가능 |
| **플러그인 및 커넥터** | Microsoft Fabric, SharePoint, Pinecone, Qdrant 등 데이터/메모리 서비스 연결 가능 |
...
에이전트 생성은 추론 서비스(LLM Provider), AI 에이전트가 따를 지침 세트, `name`을 정의해서 이루어져요:
