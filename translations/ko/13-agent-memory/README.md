...
AI 에이전트의 독특한 이점에 대해 논의할 때 주로 두 가지가 언급돼요: 도구를 호출해서 작업을 완료하는 능력과 시간이 지남에 따라 개선되는 능력이에요. 메모리는 사용자에게 더 나은 경험을 제공할 수 있는 자기 개선 에이전트를 만드는 데 핵심이에요.

이번 강의에서는 AI 에이전트의 메모리가 무엇인지, 이를 어떻게 관리하고 활용할 수 있는지 살펴볼게요.

## 이번 강의 내용

- **AI 에이전트 메모리 이해하기**: 메모리가 뭐고 에이전트에게 왜 중요한지
- **메모리 구현 및 저장**: 단기/장기 메모리에 초점을 맞춘 실질적인 방법
- **AI 에이전트를 자기 개선 가능하게 만들기**: 메모리가 과거 상호작용에서 학습하고 시간에 따라 개선하게 하는 방법

## 학습 목표

이 강의를 마치면:

- **AI 에이전트 메모리의 다양한 유형을 구분**: 작업 메모리, 단기 메모리, 장기 메모리, 페르소나 및 에피소드 메모리
- **AI 에이전트의 단기/장기 메모리 구현 및 관리**: Semantic Kernel 프레임워크로 Mem0, Whiteboard 메모리, Azure AI Search 통합
- **자기 개선 AI 에이전트의 원리 이해**: 강력한 메모리 관리 시스템이 지속적인 학습과 적응에 기여하는 방식

## AI 에이전트 메모리 이해하기

기본적으로, **AI 에이전트의 메모리는 정보를 유지하고 회상할 수 있는 메커니즘**이에요. 이 정보는 대화의 특정 세부사항, 사용자 선호도, 과거 행동, 학습된 패턴일 수 있어요.

메모리가 없으면 AI 애플리케이션은 종종 상태를 유지하지 못하며 각 상호작용이 처음부터 시작돼요. 이는 에이전트가 이전의 맥락이나 선호도를 "잊어버리는" 반복적이고 답답한 사용자 경험을 초래해요.

### 메모리가 왜 중요한가?

에이전트의 지능은 과거 정보를 회상하고 활용하는 능력과 깊이 연결돼 있어요. 메모리는 에이전트를 다음과 같이 만들어요:

| 특성 | 설명 |
|------|------|
| **반성적** | 과거 행동과 결과에서 학습 |
| **상호작용적** | 진행 중인 대화의 맥락 유지 |
| **능동적/반응적** | 과거 데이터 기반으로 필요를 예측하거나 적절히 대응 |
| **자율적** | 저장된 지식으로 더 독립적으로 작동 |

메모리 구현의 목표는 에이전트를 더 **신뢰할 수 있고 능력 있는** 존재로 만드는 거예요.

### 메모리의 유형

| 유형 | 설명 | 예시 |
|------|------|------|
| **작업 메모리** | 단일 작업 중 사용하는 임시 메모지. 즉각적인 정보 보유 | 여행 예약 에이전트가 현재 요청("파리로 여행 예약") 캡처 |
| **단기 메모리** | 단일 대화/세션 동안 정보 유지. 이전 턴 참조 가능 | "거기"가 같은 대화에서 "파리" 의미 이해 |
| **장기 메모리** | 여러 대화/세션에 걸쳐 정보 유지. 개인화에 중요 | 사용자 선호도, 과거 상호작용, 일반적 지식 기억 |
