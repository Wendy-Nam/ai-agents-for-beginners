...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-30T13:45:22+00:00",
...
AI 에이전트가 실험적 프로토타입에서 실제 애플리케이션으로 전환됨에 따라, 이들의 행동을 이해하고 성능을 모니터링하며 출력물을 체계적으로 평가하는 능력이 중요해지고 있어요.

## 학습 목표

이 강의를 마치면:

- 에이전트 관찰 가능성과 평가의 핵심 개념 이해
- 에이전트의 성능, 비용, 효과성을 개선하는 기술
- AI 에이전트를 체계적으로 평가하는 방법과 평가 대상
- AI 에이전트를 프로덕션에 배포할 때 비용을 제어하는 방법
- AutoGen으로 구축된 에이전트를 계측하는 방법

이 강의의 목표는 "블랙박스" 에이전트를 투명하고 관리 가능하며 신뢰할 수 있는 시스템으로 전환할 수 있는 지식을 제공하는 거예요.

> 💡 **참고:** 안전하고 신뢰할 수 있는 AI 에이전트를 배포하는 게 중요해요. [신뢰할 수 있는 AI 에이전트 구축](./06-building-trustworthy-agents/README.md) 강의도 확인해 보세요.

## 트레이스와 스팬

[Langfuse](https://langfuse.com/)나 [Azure AI Foundry](https://learn.microsoft.com/en-us/azure/ai-foundry/what-is-azure-ai-foundry) 같은 관찰 가능성 도구는 에이전트 실행을 트레이스와 스팬으로 나타내요.

| 용어 | 설명 |
|------|------|
| **트레이스(Trace)** | 시작부터 끝까지의 전체 에이전트 작업 (예: 사용자 쿼리 처리) |
| **스팬(Span)** | 트레이스 내의 개별 단계 (예: LLM 호출 또는 데이터 검색) |

![Langfuse의 트레이스 트리](https://langfuse.com/images/cookbook/example-autogen-evaluation/trace-tree.png)

관찰 가능성이 없으면 AI 에이전트는 "블랙박스"처럼 느껴질 수 있어요. 내부 상태와 추론이 불투명해서 문제 진단이나 성능 최적화가 어려워요. 관찰 가능성을 통해 에이전트는 "글래스박스"가 되어 투명성을 제공해요.

## 프로덕션 환경에서 관찰 가능성이 중요한 이유

AI 에이전트를 프로덕션으로 전환하면 새로운 도전 과제가 발생해요. 관찰 가능성은 필수적인 역량이에요:

| 이유 | 설명 |
|------|------|
| **디버깅 및 근본 원인 분석** | 에이전트 실패 시 오류의 원인을 정확히 찾아내는 트레이스 제공 |
| **지연 시간 및 비용 관리** | LLM 호출을 정확히 추적해서 느리거나 비용이 많이 드는 작업 식별 |
| **신뢰, 안전, 규정 준수** | 에이전트의 행동과 결정을 감사할 수 있는 기록 제공 |
| **지속적인 개선 루프** | 실제 환경 모니터링으로 개선이 필요한 영역 식별, 모델 미세 조정 |

## 추적해야 할 주요 메트릭

에이전트의 행동을 모니터링하고 이해하려면 다양한 메트릭을 추적해야 해요:

| 메트릭 | 설명 |
|--------|------|
| **지연 시간(Latency)** | 에이전트 응답 속도. 긴 대기 시간은 UX에 부정적 영향 |
| **비용(Costs)** | 에이전트 실행당 비용. 도구 빈번 사용이나 여러 프롬프트 호출로 비용 증가 |
| **요청 오류(Request Errors)** | API 오류나 도구 호출 실패. 폴백이나 재시도 설정 필요 |
| **사용자 피드백** | 명시적 평가(👍/👎, ⭐1-5점)나 텍스트 코멘트 |
| **암묵적 사용자 피드백** | 즉각적인 질문 재구성, 반복된 쿼리, 재시도 버튼 클릭 등 |
| **정확성(Accuracy)** | 올바르거나 바람직한 출력 생성 빈도. 성공 정의가 첫 단계 |
| **자동화된 평가 메트릭** | LLM으로 출력 유용성/정확성 평가, RAGAS, LLM Guard 같은 라이브러리 활용 |
